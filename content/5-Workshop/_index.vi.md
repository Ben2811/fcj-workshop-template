---
title: "Workshop"
date: 2024-01-01
weight: 5
chapter: false
pre: " <b> 5. </b> "
---

# Triển khai PeriodIQ bằng AWS CLI - Theo vai trò từng người

#### Tổng quan

**PeriodIQ** là một "huấn luyện viên tự động" serverless do một team 5 người xây dựng, mỗi người phụ trách 3-4 dịch vụ AWS (xem [Proposal](../2-proposal/)). Phần này được tổ chức theo đúng cơ cấu team đó - mỗi mục ứng với một người. Trong báo cáo này, tôi chỉ ghi chi tiết vai trò của mình - **Chương Tử Luân (Admin Panel & Data)**, gồm **Amazon DynamoDB, Lambda Admin API, và API Gateway**, cùng với **Admin Panel frontend** xây trên các dịch vụ đó - mọi bước đều làm bằng **AWS CLI** và có ảnh chụp màn hình thật minh hoạ.

#### Nội dung

1. [Tổng quan về workshop](5.1-Workshop-overview/)
2. [Chuẩn bị](5.2-Prerequiste/)
3. [Lê Hoài Huân - Auth & User Profile](5.3-Nguoi1-Auth/)
4. [Trần Anh Tài - Rule Engine & Sinh giáo án](5.4-Nguoi2-RuleEngine/)
5. [Lê Hữu Duy Hoàng - Tiến trình & Async Notification](5.5-Nguoi3-Async/)
6. [Chương Tử Luân - Admin Panel & Data](5.6-Nguoi4-Admin/) *(vai trò của tôi - ghi chi tiết bên dưới)*
7. [Phạm Văn Sỹ - CI/CD & Monitoring](5.7-Nguoi5-CICD/) *(vai trò của đồng đội - đưa vào để làm rõ bối cảnh)*
8. [Dọn dẹp tài nguyên](5.8-Cleanup/)
