...
> **CLI Note:** mọi giá trị trên trang này - tên/retention log group, trạng thái alarm, độ sâu queue - đều lấy từ lệnh `aws logs` / `aws cloudwatch` / `aws sqs` gọi trực tiếp trên account đang chạy. Phát hiện về DLQ không phải do bạn ấy dàn dựng cho workshop này - đó là một tình trạng đã tồn tại sẵn trong hệ thống đang được giám sát.

#### Cùng dữ liệu đó, qua dashboard admin bạn ấy đã xây dựng
