...
#### The same execution, through the admin dashboard he built
...
This lists real pipeline executions by commit - including several from his own `Sy-2` branch merges (`#48`, `#47`, `#46`, `#45`, `#44`) alongside teammates' PRs (`#49` from `hoang`, `#43` from `Tai`, `#42` from `Huan`). The overall success rate shown (70%, 14/20) and average duration (6m 53s) are computed from this same real execution history - and the failed runs are shown honestly rather than hidden.
