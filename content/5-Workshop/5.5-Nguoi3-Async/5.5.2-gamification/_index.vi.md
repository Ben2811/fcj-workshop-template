...
Thay vì sử dụng cronjob tính toán chậm trễ, bạn ấy quyết định cập nhật XP và Streak theo thời gian thực (Real-time). Dữ liệu này được lưu trữ trong Entity `Progress.cs` của DynamoDB, bao gồm các trường chính như `XP`, `CurrentStreak`, `LongestStreak`, và `LastWorkoutDate`.
